...
- **Real bug found and fixed (2026-09-26): editing an already-linked institution's modules could fail outright with `400 "Unknown module key: exams"` — reported live with a real institution (`sams_university`) that had 24 stored module keys against a 22-entry catalog ("24 of 22 selected").** Root cause: `institution-modules-dialog.tsx`'s `moduleKeys` state was seeded directly from the institution's raw stored array (`institution-modules-dialog.tsx`'s `useState(new Set(initialInstitution?.moduleKeys ?? []))` and `handleAdd`'s equivalent) with no check against the current, real catalog — so a few legacy keys from an earlier, now-defunct catalog version (`exams`, `reports`, `sms-integration`, `ussd-services` — none in today's 22-key `ModuleCatalog`, see `backend/CLAUDE.md`) sat invisibly "selected" forever (nothing renders a checkbox for a key the fetched catalog doesn't have) and got faithfully resubmitted on every save, which the backend correctly rejects. **Fix was deliberately not a `useEffect` pruning `moduleKeys`** — the lint config here (`react-hooks/set-state-in-effect`) flags exactly that pattern, and rightly so: it's paying for a cascading re-render just to keep a derived display in sync. Instead, `selectedValidKeys = catalog.filter((m) => moduleKeys.has(m.key))` is computed once per render and used everywhere that matters — the "N of {catalog.length} selected" counter, `allSelected`, and the actual save payload (`selectedValidKeys.map((m) => m.key)`) — so stale keys simply stop being counted or submitted anywhere, with zero extra state or effect. `handleAdd` filters the same way at the point it copies an institution's keys in. A successful save now shows a second toast ("Also cleared N modules no longer offered by the platform") whenever `moduleKeys.size` and `selectedValidKeys.length` actually differ, so the user isn't left wondering why their count dropped. **This is self-healing, not a one-off patch**: the fix doesn't touch the database directly — the very next real save through the now-fixed dialog naturally persists only the clean, valid set, which is exactly what happened live for `sams_university` (verified: `PATCH /institutions/:id/modules` returned `200` with `moduleKeys` now containing only its 11 real, valid selections, `modulesCount: 11` — the stale entries are gone from the stored row, not just hidden in the UI). Live-verified end-to-end against the real backend, 5/5 checks (correct counter, no error toast, success toast, `200` response, zero console errors).
...
- **A real, shell-wide layout bug found and fixed the same day (2026-09-26), reported as "this page is not sitting right" with a screenshot of `/super-admin/institutions`.** At a common laptop width (1366px), the Token Key/Status/Action columns were completely cut off past the right edge of the browser with **no way to scroll to them at all** — a real usability bug, not cosmetic, since Action is where every row's Edit/Deactivate/Delete menu lives. Root cause was in `app-shell.tsx`, not the Institutions page itself: `components/ui/table.tsx`'s `Table` already wraps every table in its own `overflow-x-auto` container (so a wide table is supposed to scroll locally within its card) — but the shell's content column (`<div className="flex min-h-0 flex-1 flex-col">` next to the fixed sidebar) was missing `min-w-0`. A flex item's default `min-width: auto` lets it refuse to shrink below its content's intrinsic width, so instead of the Table's own scroll container taking over, the _entire shell_ grew wider than the viewport and the outer `overflow-hidden` (`app-shell.tsx`'s root div) silently clipped everything past the fold — with no scrollbar anywhere to reach it. This is the single most common real-world flexbox bug (search "flexbox min-width auto" for why) and explains why _both_ "View archived (N)" and "Today's Date" also looked clipped in the reported screenshot despite each having its own `flex-wrap` — they were never actually wrapping, the whole page was just wider than the window with no way to scroll to the rest. Fixed with one line in each of the shell's two nested flex children (`min-w-0` on both the column wrapper and `<main>`) — a single, root-cause, shell-level fix rather than patching `overflow-x-auto` onto every individual admin-table page (which wouldn't have helped anyway, since the table already had it; the bug was one level up). Verified live at 1366px: every header element now fits/wraps correctly, and the Institutions table now shows its own local horizontal scroll reaching Token Key/Status/Action (confirmed by actually scrolling the table wrapper and asserting the Action kebab button becomes visible/clickable) — not just visually plausible, actually interactable. Confirmed no regression at 1920px or on `/dashboard`'s chart-heavy layout (`ResponsiveContainer` still sizes correctly — `min-w-0` only constrains the _minimum_ shrink, never the rendered size when space is available). This fix applies everywhere at once (`AppShell` is shared by every `/dashboard` and `/super-admin` page), so no other page needs the same fix repeated.
  **One follow-up bug this fix exposed, fixed the same day**: once the table's own horizontal scroll started actually working, its scrollbar rendered as the browser's chunky default (reported as "this is too big pls make it slimmer") — because `components/ui/table.tsx`'s scroll container never had the app's own thin, branded scrollbar utility applied (`scrollbar-brand-light`, already used on `<main>` and the sidebar — 6px, rounded, colored `--secondary`). It simply never mattered before, since that container never visibly scrolled. Fixed by adding `scrollbar-brand-light` to that one container in the shared `Table` component — same one-fix-covers-every-table effect as the shell fix above.
