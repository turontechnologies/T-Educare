...
export default function ProgramLevelsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader breadcrumb={["Administrator", "Program Levels"]} />

      <div>
        <h1 className="text-xl font-semibold text-primary">Program Levels</h1>
      ...
      <ProgramLevelDialog
        ...
        programLevel={editingLevel}
      />
    </div>
  );
}

function ProgramLevelTable({
  onEdit,
}: {
  ...
  const [pendingArchive, setPendingArchive] = useState<ProgramLevel | null>(
    null,
  );

  const baseList = useMemo(
    () =>
      programLevels.filter((level) =>
        view === "archived" ? level.archivedAt : !level.archivedAt,
      ),
    [programLevels, view],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return baseList.filter(
      (level) =>
        level.levelCode.toLowerCase().includes(query) ||
  ...
  const archivedCount = programLevels.filter((l) => l.archivedAt).length;
  ...
  return (
    <>
      ...
      <Card className="animate-in fade-in slide-in-from-bottom-2 fill-mode-both duration-500">
        <CardHeader className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 text-sm">
            <Label
              htmlFor="program-level-search"
              className="text-muted-foreground"
            >
              Filter:
            </Label>
            <Input
              id="program-level-search"
              value={search}
              onChange={(event) => {
            ...
            <TableBody>
                ...
                >
                  ...
                  <TableCell className="text-right">
                    {view === "active" ? (
                      <DropdownMenu>
                        <DropdownMenuTrigger
                          aria-label={`Actions for ${level.levelCode}`}
                        ...
                        <DropdownMenuContent align="end" className="w-40">
                          ...
                          <DropdownMenuItem
                            variant="destructive"
                            onClick={() => setPendingArchive(level)}
                          >
              ...
              {paginated.length === 0 && (
                <TableRow>
                  ...
                  >
                    {view === "archived"
                      ? "No archived program levels."
                      : "No program levels match your search."}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Delete this program level?"
