...
export default function DepartmentManagementPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader breadcrumb={["Administrator", "Department Management"]} />
...
function DepartmentTable({
  ...
  const baseList = useMemo(
    () =>
      departments.filter((department) =>
        view === "archived" ? department.archivedAt : !department.archivedAt,
      ),
    [departments, view],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return baseList.filter(
      (department) =>
        ...
        facultyName(department.facultyId).toLowerCase().includes(query) ||
        schoolName(department.schoolId).toLowerCase().includes(query),
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [baseList, search, faculties, schools]);
  ...
  const archivedCount = departments.filter((d) => d.archivedAt).length;
            ...
            <TableBody>
              {paginated.map((department, index) => (
                ...
                >
                  ...
                  <TableCell className="text-muted-foreground">
                    {facultyName(department.facultyId)}
                  </TableCell>
                  <TableCell className="text-muted-foreground">
                    {schoolName(department.schoolId)}
                  </TableCell>
                    ...
                    {view === "active" ? (
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-40">
                          ...
                          <DropdownMenuItem
                            variant="destructive"
                            onClick={() => setPendingArchive(department)}
                          >
